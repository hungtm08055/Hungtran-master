<?php
    ...
    $list_id = $_POST['id_list']; // lấy lại id của bảng list từ phương thức GET bên list
                
    $task_listId = $db->prepare("SELECT * FROM task WHERE listId = ?"); // lay userId cua bang list
    ...
    foreach($task_listId as $task_row)
    {
        ...
        if($task_row['status'] == 1) // so sánh 2 id từ bảng lisst và khóa ngoại listId của bảng task
        {
            echo '<div class="task-list-Completed">'
            .'<div class="tasklist-all">'
                . '<div class="tasklist-body" id="tasklist-body1" condition=1 id_list='.$task_row['listId'].' id_task='.$task_row['id'].' status=1 unstatus=0 star=1 unstar=0 date_create='.$task_row['createdate'].'>'
                    .'<div class="tasklist-icon">'
                    // truyền 3 thông số list_id,task_id,status sang trang xulyStatus.php
                    ...
                    .'</div>'
                    .'<div class="task-deleted-text1 line-through" id_task="'.$task_row['id'].'" id_list="'.$task_row['listId'].'">';
                    // .'<a href="hienthi.php?id_task='.$task_row['id'].'&id_list='.$list_id.'">'; 
                        echo '<div id="task-completed-text">';
                            echo $task_row['title']; // hiển thị title của task
                            $task_id = $_GET['id_task']; // id của task
                            $_SESSION['id_task'] = $task_id; // lưu vào session
                        echo '</div>';                       
                        echo '<div class="task-deleted-text">'
                            .'<div class="task-deleted-text2">'.$task_row["createdate"].'</div>' // hiển thị trường createdate                                                   
                        . '</div>'
                        // .'</a>'
                    . '</div>';
                    echo '<div class="duedate">';                    
                        ...
                        $_SESSION['origin_time'] = $origin_time;                                             
                        if($origin_time == $current_time && $origin_date == $current_date)
                        {
                            echo '<audio controls autoplay class="none">
                                <source src="../image/notification.mp3" type="audio/mpeg">
                            </audio>';   
